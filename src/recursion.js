...
var arraySum = function(array) {
  if(array.length === 0){
    ...
    return array.reduce(function(total, element){
      return total + (Array.isArray(element) ? arraySum(element) : element);
    }, 0);
  }
};
...
var createArray = function(str) {
  let strArr = [];
  if(str.length > 0){
    return str.length === 1 ? strArr.concat(str[0]): strArr.concat(str[0]).concat(createArray(str.substring(1, str.length)));      
  }
};
...
var reverseArr = function(array) {
  let revArr = [];
  if(array.length > 0){
    return array.length === 1 ? revArr.concat(array[0]): revArr.concat(array[array.length-1]).concat(reverseArr(array.slice(0, array.length-1)));      
  }
};
